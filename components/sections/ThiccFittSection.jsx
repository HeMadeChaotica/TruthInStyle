...
export default function ThiccFittSection() {
  ...
  const [dailyQuote, setDailyQuote] = useState(optionRegistry.thiccFitt.quoteOfDay[0]?.text || '');
  ...
  useEffect(() => {
    ...
    if (todayId) {
      setDailyQuote((quoteBank.find((q) => q.id === todayId) || quoteBank[0]).text);
      return;
    }
    const remaining = quoteBank.filter((q) => !history.usedQuoteIds.includes(q.id) && q.id !== history.lastQuoteId);
    if (remaining.length === 0) {
      history.usedQuoteIds = [];
      history.cycle += 1;
    }
    const pool = quoteBank.filter((q) => !history.usedQuoteIds.includes(q.id) && q.id !== history.lastQuoteId);
    const nextQuote = pool[Math.floor(Math.random() * pool.length)] || quoteBank.find((q) => q.id !== history.lastQuoteId) || quoteBank[0];
  ...
  useEffect(() => {
    publishThiccFittSessionProof({
      ...
      coreWork: '15 MINUTES',
      coreFocus: state.coreFocus,
      coreTime: state.coreMinutes,
      cardioType: state.cardio.type,
      cardioDuration: state.cardio.duration,
      cardioIntensity: state.cardio.intensity,
      cardioLocation: state.cardio.location,
      cardioNotes: state.cardio.notes,
