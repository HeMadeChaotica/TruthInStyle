...
export default function ThiccFittSection() {
  ...
  return <section className="tf-page"><div className="tf-overlay" />
    ...
    <main className="tf-grid">
      ...
      <section className="tf-panel tf-left-stack"><section className="tf-panel tf-stats"><h2>STATS</h2><div className="tf-stats-scroll"><div className="tf-stats-grid">{statFields.map(([k, l]) => <label key={k} className="tf-labeled"><span>{l}</span><input value={state.form[k]} onChange={(e) => updateField(k, e.target.value)} /></label>)}</div><textarea value={state.form.bodyNotes} onChange={(e) => updateField('bodyNotes', e.target.value)} placeholder="NOTES" /></div></section><section className="tf-panel tf-vault"><h2>THE VAULT</h2><div className="tf-vault-scroll">{state.vaultRows.map((row, i) => <div key={i} className="tf-vault-row"><select value={row.compound} onChange={(e) => updateVault(i, 'compound', e.target.value)}><option value="">COMPOUND</option>{opts.roidCompound.map((o) => <option key={o}>{o}</option>)}</select><select value={row.ester} onChange={(e) => updateVault(i, 'ester', e.target.value)}><option value="">ESTER</option>{opts.roidEster.map((o) => <option key={o}>{o}</option>)}</select><select value={row.amount} onChange={(e) => updateVault(i, 'amount', e.target.value)}><option value="">AMOUNT</option>{opts.roidAmount.map((o) => <option key={o}>{o}</option>)}</select><div className="tf-inline-fill"><span>SHOT</span><input value={row.shotOf} onChange={(e) => updateVault(i, 'shotOf', e.target.value)} /><span>OF</span><input value={row.cycleLength} onChange={(e) => updateVault(i, 'cycleLength', e.target.value)} /></div><select value={row.sensitivity} onChange={(e) => updateVault(i, 'sensitivity', e.target.value)}><option value="">SENSITIVITY</option>{opts.roidSensitivity.map((o) => <option key={o}>{o}</option>)}</select><div className="tf-inline-fill tf-span"><span>CYCLE WEEK</span><input value={row.weekOf} onChange={(e) => updateVault(i, 'weekOf', e.target.value)} /><span>OF</span><input value={row.amount} onChange={(e) => updateVault(i, 'amount', e.target.value)} /></div></div>)}
        <div className="tf-pump-wrap"><button type="button" className="tf-pump-trigger" onClick={sendToAssurer} onDoubleClick={resetSends} disabled={!canSend} aria-label="Pump It send trigger"><img src="/backgrounds/THICC-FITT/pump-it.PNG" alt="PUMP-IT GLYPH" /></button></div></div></section><section className="tf-panel tf-cardio"><h2>CARDIO</h2><div className="tf-row"><select value={state.form.cardioType} onChange={(e) => updateField('cardioType', e.target.value)}><option value="">TYPE</option>{opts.roidCardioType.map((o) => <option key={o}>{o}</option>)}</select><select value={state.form.cardioDuration} onChange={(e) => updateField('cardioDuration', e.target.value)}><option value="">DURATION</option>{opts.roidCardioDuration.map((o) => <option key={o}>{o}</option>)}</select><select value={state.form.cardioIntensity} onChange={(e) => updateField('cardioIntensity', e.target.value)}><option value="">INTENSITY</option>{opts.roidIntensity.map((o) => <option key={o}>{o}</option>)}</select></div><input placeholder="CARDIO LOCATION" value={state.form.cardioLocation} onChange={(e) => updateField('cardioLocation', e.target.value)} /><textarea value={state.form.cardioNotes} onChange={(e) => updateField('cardioNotes', e.target.value)} placeholder="NOTES" /></section></section>
    </main>
  </section>;
}
