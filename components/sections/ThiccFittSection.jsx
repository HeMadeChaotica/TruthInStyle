  ...
  return (
    <section className="tf-page">
      ...
      <header className="tf-header tf-panel">
        <h1>THICC.FITT <span className="tf-crown" role="img" aria-label="Giraffe king crown">👑🦒</span></h1>
        ...
        <Link href="/its-getting-thicc" className="tf-client-link"><img src="/ui/glyphs/triggers/glyph-crystal-dumbbell.png" alt="Crystal dumbbell trigger" /></Link>
      </header>
      ...
      <main className="tf-grid">
        ...
        <section className="tf-panel tf-body"><h2>6. MEDIA</h2><label className="tf-upload">UPLOAD<input type="file" multiple accept="image/*" onChange={(e) => setMediaPreview(Array.from(e.target.files || []))} /></label><div className="tf-preview">{previewUrls.map((u) => <img key={u} src={u} alt="Preview" />)}</div></section>
      </main>
    </section>
  );
}
