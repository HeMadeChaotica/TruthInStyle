...
const formatMomentDate = (rawValue) => {
  if (!rawValue) return '--';
...
export default function ThiccFittSection() {
  ...
  const momentDetails = useMemo(() => ({
    trainingFor: state?.stageCall?.trainingFor || 'MOMENT NOT SET',
    targetDate: formatMomentDate(state?.stageCall?.targetDate),
    location: state?.stageCall?.location || '--',
    description: state?.stageCall?.momentDescription || 'SET THIS LATER IN CLOCK.IT'
  }), [state]);
