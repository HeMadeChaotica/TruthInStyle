...
const emptyExercise = { exercise: '', weight: '', sets: '', reps: '', notes: '' };
  ...
  return (
    <section className="tf-page">
      ...
      <header className="tf-header tf-panel">
        <img className="tf-title-glyph" src="/backgrounds/THICC-FITT/thicc-title.jpeg" alt="THICC.FITT" />
        <Link href="/its-getting-thicc" className="tf-client-link"><img src="/ui/glyphs/triggers/glyph-crystal-dumbbell.png" alt="Crystal dumbbell" /></Link>
      ...
      <main className="tf-grid">
        <section className="tf-panel tf-exercises"><h2>EXERCISE LOG</h2>{exerciseRows.map((row, i) => <div key={i} className="tf-row"><input placeholder="EXERCISE" value={row.exercise} onChange={(e)=>updateExercise(i,'exercise',e.target.value)} /><input placeholder="WEIGHT" value={row.weight} onChange={(e)=>updateExercise(i,'weight',e.target.value)} /><input placeholder="SETS" value={row.sets} onChange={(e)=>updateExercise(i,'sets',e.target.value)} /><input placeholder="REPS" value={row.reps} onChange={(e)=>updateExercise(i,'reps',e.target.value)} /><input placeholder="NOTES" value={row.notes} onChange={(e)=>updateExercise(i,'notes',e.target.value)} /></div>)}<button type="button" onClick={addExercise}>+ ADD EXERCISE</button></section>
        ...
        <section className="tf-panel tf-notes"><h2>SO HOW YOU DOIN 🫦⁉️</h2><div className="tf-notes-stack"><select value={form.notesPrompt} onChange={(e)=>updateField('notesPrompt', e.target.value)}><option>I WON’T SUM MO COACH GIMME THAT</option></select><textarea value={form.notesText} onChange={(e)=>updateField('notesText', e.target.value)} placeholder="WRITE YOUR THOUGHTS HERE..." /></div></section>
