...
const VAULT_ROW_COUNT = 4;
const createExerciseRow = () => ({ exercise: '', weight: '', reps: '', sets: '', failure: '', rest: '' });
...
const createVaultRow = () => ({ compound: '', ester: '', amount: '' });
const normalizeVaultRows = (rows, legacyVault = {}) => {
  const source = Array.isArray(rows) && rows.length ? rows : [legacyVault];
  return Array.from({ length: VAULT_ROW_COUNT }, (_, index) => {
    const row = source[index] || {};
    return {
      compound: String(row.compound ?? ''),
      ester: String(row.ester ?? ''),
...
const initialState = { control: { gymLocation: '', arrivalTime: '', workoutLength: '', seasonPhase: '', sorenessRecovery: '', prepStatus: '' }, exerciseRows: Array.from({ length: EXERCISE_COUNT }, createExerciseRow), core: { focus: '', circuit: '', rounds: '', repScheme: '', format: 'BODYWEIGHT', completed: '' }, cardio: { type: '', duration: '', intensity: '', location: '', notes: '', weeklyGoal: '3 SESSIONS / 60 MIN', weeklyDone: '' }, vault: { compound: '', ester: '', amount: '', shotCurrent: '', shotTotal: '', sensitivity: '', cycleWeekCurrent: '', cycleWeekTotal: '' }, vaultRows: Array.from({ length: VAULT_ROW_COUNT }, createVaultRow), body: Object.fromEntries(bodyRows.map(([k]) => [k, { today: '', lastWeek: '', change: '' }])), bodyNotes: '', stageCall: { months: '', days: '', hours: '', minutes: '', seconds: '', stageDescription: '', posingMinutes: '', mandatoryRoundPracticed: '', strongestPose: '', weakestPose: '', transitions: '', posingFatigue: '', coachSelfNotes: '' }, soHowYouDoin: optionRegistry.thiccFitt.soHowYouDoin[0], soHowYouDoinNotes: '', photo: { progressPhotoRef: '', gymPhotoRef: '' }, weeklyTrackers: { weekStart: getWeekStartKey(), byDay: Object.fromEntries(WEEK_DAYS.map((d) => [d, { training: { gymMinutes: 0, cardioMinutes: 0 }, caffeineMg: '', sleep: createSleepEntry() }])) }, sessionCompleted: '' };
...
export default function ThiccFittSection() {
  ...
  const yesNoOptions = useClockItOptions(CLOCK_IT_KEYS.yesNo);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const vaultRows = normalizeVaultRows(parsed.vaultRows, parsed.vault);
        const safe = {
          ...parsed,
          exerciseRows: normalizeExerciseRows(parsed.exerciseRows),
          vault: { ...initialState.vault, ...(parsed.vault || {}), ...vaultRows[0] },
          vaultRows,
  ...
  const updateVaultRow = (i, key, value) => setState((p) => {
    const vaultRows = p.vaultRows.map((row, index) => (index === i ? { ...row, [key]: value } : row));
    return { ...p, vaultRows, vault: i === 0 ? { ...p.vault, ...vaultRows[0] } : p.vault };
  });
  ...
  const shelves = [
    { id: 'A', columns: 1, className: 'tf30-shelf-a', panels: [{ id: 'entry-gate', token: 'compact', className: 'tf30-training-panel', content: <><h2>IN THE BEGINNING...</h2><div className="tf30-grid-2"><label>GYM<input value={state.control.gymLocation} onChange={(e) => update('control', 'gymLocation', e.target.value)} /></label><label>ARRIVAL<input type="time" value={state.control.arrivalTime} onChange={(e) => update('control', 'arrivalTime', e.target.value)} /></label><label>SEASON / PHASE<select required value={state.control.seasonPhase} onChange={(e) => update('control', 'seasonPhase', e.target.value)}><option value="" aria-label="Blank selection"></option>{seasonOptions.map((o) => <option key={o}>{o}</option>)}</select></label><label>SORENESS LEVEL<select required value={state.control.sorenessRecovery} onChange={(e) => update('control', 'sorenessRecovery', e.target.value)}><option value="" aria-label="Blank selection"></option>{sorenessOptions.map((o) => <option key={o}>{o}</option>)}</select></label><label>WORKOUT LENGTH<select required value={state.control.workoutLength} onChange={(e) => update('control', 'workoutLength', e.target.value)}><option value="" aria-label="Blank selection"></option>{workoutDurationOptions.map((o) => <option key={o}>{o}</option>)}</select></label><label>PREP STATUS<select required value={state.control.prepStatus} onChange={(e) => update('control', 'prepStatus', e.target.value)}><option value="" aria-label="Blank selection"></option>{prepStatusOptions.map((o) => <option key={o}>{o}</option>)}</select></label></div></> }] },
    { id: 'B', columns: 1, className: 'tf30-shelf-b', panels: [{ id: 'iron-ledger', token: 'tall', content: <><h2>IRON.LEDGER</h2><div className="tf30-table-head"><span>#</span><span>EXERCISE</span><span>WEIGHT</span><span>REPS</span><span>SETS</span><span>FAILURE</span><span>REST</span></div>{state.exerciseRows.map((r, i) => <div className="tf30-table-row" key={i}><span>{i + 1}</span><select value={r.exercise} onChange={(e) => updateExercise(i, 'exercise', e.target.value)}><option value="">SELECT</option>{exerciseOptions.map((o)=><option key={o}>{o}</option>)}</select><select value={r.weight} onChange={(e) => updateExercise(i, 'weight', e.target.value)}><option value="">--</option>{exerciseWeightOptions.map((o)=><option key={o.value} value={o.value}>{o.label}</option>)}</select><select value={r.reps} onChange={(e) => updateExercise(i, 'reps', e.target.value)}><option value="">--</option>{exerciseRepOptions.map((o)=><option key={o.value} value={o.value}>{o.label}</option>)}</select><select value={r.sets} onChange={(e) => updateExercise(i, 'sets', e.target.value)}><option value="">--</option>{exerciseSetOptions.map((o)=><option key={o.value} value={o.value}>{o.label}</option>)}</select><select value={r.failure === 'Y' ? 'YES' : r.failure === 'N' ? 'NO' : r.failure || ''} onChange={(e) => updateExercise(i, 'failure', e.target.value)}><option value="">--</option>{yesNoOptions.map((option) => <option key={option}>{option}</option>)}</select><select value={r.rest || ''} onChange={(e) => updateExercise(i, 'rest', e.target.value)}><option value="">--</option>{exerciseRestOptions.map((o)=><option key={o.value} value={o.value}>{o.label}</option>)}</select></div>)}</> }] },
    { id: 'C', columns: 2, className: 'tf30-shelf-c', panels: [{ id: 'da-vault', token: 'medium', content: <><h2>DA.VAULT</h2><div className="tf30-vault-table-head"><span>#</span><span>COMPOUND</span><span>ESTER / FORM</span><span>AMOUNT</span></div>{state.vaultRows.map((row, i) => <div className="tf30-vault-row" key={i}><span>{i + 1}</span><select aria-label={`DA.VAULT row ${i + 1} compound`} value={row.compound} onChange={(e) => updateVaultRow(i, 'compound', e.target.value)}><option value="">--</option>{compoundOptions.map((o) => <option key={o}>{o}</option>)}</select><select aria-label={`DA.VAULT row ${i + 1} ester or form`} value={row.ester} onChange={(e) => updateVaultRow(i, 'ester', e.target.value)}><option value="">--</option>{esterOptions.map((o) => <option key={o}>{o}</option>)}</select><select aria-label={`DA.VAULT row ${i + 1} amount`} value={row.amount} onChange={(e) => updateVaultRow(i, 'amount', e.target.value)}><option value="">--</option><optgroup label="CC">{amountCcOptions.map((o) => <option key={o}>{o}</option>)}</optgroup><optgroup label="MG">{amountMgOptions.map((o) => <option key={o}>{o}</option>)}</optgroup></select></div>)}</> }, { id: 'war-cry', token: 'compact', className: 'tf30-quote-panel', content: <><h2>THE WAR CRY</h2><div className="tf30-war-cry-frame"><blockquote>{dailyQuote?.text}</blockquote><div className="tf30-quote-meta">— {dailyQuote?.author} · {dailyQuote?.source}</div></div></> }] },
  ...
  const tiles = [
    ...
    { id: 'iron-ledger', title: 'IRON.LEDGER', alwaysOpen: true, summary: firstExercise ? `${firstExercise.exercise}\n${firstExercise.sets || '—'} SETS · ${firstExercise.reps || '—'} REPS · ${firstExercise.weight || '—'}` : 'OPEN TO LOG TODAY’S WORKOUT', content: panelById['iron-ledger'] },
    { id: 'da-vault', title: 'DA.VAULT', alwaysOpen: true, summary: 'FOUR ROW VAULT LOG', content: panelById['da-vault'] },
