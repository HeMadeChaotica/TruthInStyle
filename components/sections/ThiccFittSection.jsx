...
export default function ThiccFittSection() {
  ...
  const momentStripText = `MOMENT: ${momentDetails.trainingFor || 'MOMENT NOT SET'} • TARGET: ${momentDetails.targetDate || '--'} • ${momentDetails.location || momentDetails.description || 'SET THIS LATER IN CLOCK.IT'}`;
  ...
  const shelves = [
    { id: 'A', columns: 1, className: 'tf30-shelf-a', panels: [{ id: 'moment-clock', token: 'compact', className: 'tf30-moment-clock-panel', content: <><h2>THE MOMENT CLOCK</h2><div className="tf30-moment-clock-digital" aria-live="off"><div className="tf30-moment-clock-unit"><span className="tf30-moment-clock-value">{state.stageCall.months || '00'}</span><span className="tf30-moment-clock-label">MONTHS</span></div><div className="tf30-moment-clock-unit"><span className="tf30-moment-clock-value">{state.stageCall.days || '00'}</span><span className="tf30-moment-clock-label">DAYS</span></div><div className="tf30-moment-clock-unit"><span className="tf30-moment-clock-value">{state.stageCall.minutes || '00'}</span><span className="tf30-moment-clock-label">MINUTES</span></div><div className="tf30-moment-clock-unit"><span className="tf30-moment-clock-value">{state.stageCall.seconds || '00'}</span><span className="tf30-moment-clock-label">SECONDS</span></div></div><div className="tf30-moment-strip" role="note" aria-label="Moment mission details">{momentStripText}</div></> }] },
