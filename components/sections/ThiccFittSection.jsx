...
export default function ThiccFittSection() {
  ...
  const shelves = [
    { id: 'A', columns: 2, className: 'tf30-shelf-a', panels: [{ id: 'entry-gate', token: 'compact', className: 'tf30-training-panel', content: <><h2>THE ENTRY GATE</h2><div className="tf30-grid-2"><label>GYM<input value={state.control.gymLocation} onChange={(e) => update('control', 'gymLocation', e.target.value)} /></label><label>ARRIVAL<input type="time" value={state.control.arrivalTime} onChange={(e) => update('control', 'arrivalTime', e.target.value)} /></label><label>SEASON / PHASE<select value={state.control.seasonPhase} onChange={(e) => update('control', 'seasonPhase', e.target.value)}><option value="">--</option>{optionRegistry.thiccFitt.roidSeason.map((o) => <option key={o}>{o}</option>)}</select></label><label>SORENESS LEVEL<select value={state.control.sorenessRecovery} onChange={(e) => update('control', 'sorenessRecovery', e.target.value)}><option value="">--</option>{optionRegistry.thiccFitt.sorenessLevel.map((o) => <option key={o}>{o}</option>)}</select></label><label>WORKOUT LENGTH<select value={state.control.workoutLength} onChange={(e) => update('control', 'workoutLength', e.target.value)}><option value="">--</option>{optionRegistry.thiccFitt.roidWorkoutDuration.map((o) => <option key={o}>{o}</option>)}</select></label><label>PREP STATUS<select value={state.control.prepStatus} onChange={(e) => update('control', 'prepStatus', e.target.value)}><option value="">--</option>{optionRegistry.thiccFitt.prepStatus.map((o) => <option key={o}>{o}</option>)}</select></label></div></> }] },
    { id: 'B', columns: 1, className: 'tf30-shelf-b', panels: [{ id: 'iron-ledger', token: 'tall', content: <><h2>IRON.LEDGER</h2><div className="tf30-table-head"><span>#</span><span>EXERCISE</span><span>WEIGHT</span><span>REPS</span><span>SETS</span><span>FAILURE</span><span>REST</span></div>{state.exerciseRows.map((r, i) => <div className="tf30-table-row" key={i}><span>{i + 1}</span><input value={r.exercise} onChange={(e) => updateExercise(i, 'exercise', e.target.value)} /><input value={r.weight} onChange={(e) => updateExercise(i, 'weight', e.target.value)} /><input value={r.reps} onChange={(e) => updateExercise(i, 'reps', e.target.value)} /><input value={r.sets} onChange={(e) => updateExercise(i, 'sets', e.target.value)} /><select value={r.failure || 'N'} onChange={(e) => updateExercise(i, 'failure', e.target.value)}><option>Y</option><option>N</option></select><input value={r.rest || ''} onChange={(e) => updateExercise(i, 'rest', e.target.value)} /></div>)}<div className="tf30-core-strip"><h3>CORE / AB FINISHER</h3><div className="tf30-core-grid"><label>FOCUS<input value={state.core.focus} onChange={(e) => update('core', 'focus', e.target.value)} /></label><label>CIRCUIT<input value={state.core.circuit} onChange={(e) => update('core', 'circuit', e.target.value)} /></label><label>ROUNDS<input value={state.core.rounds} onChange={(e) => update('core', 'rounds', e.target.value)} /></label><label>REP SCHEME<input value={state.core.repScheme} onChange={(e) => update('core', 'repScheme', e.target.value)} /></label><label>FORMAT<input value={state.core.format} onChange={(e) => update('core', 'format', e.target.value)} /></label><label>COMPLETED<input value={state.core.completed} onChange={(e) => update('core', 'completed', e.target.value)} /></label><label>SESSION TOTAL (MIN)<input value={state.control.workoutLength} onChange={(e) => update('control', 'workoutLength', e.target.value)} /></label><label>SESSION COMPLETED<input value={state.sessionCompleted} onChange={(e) => setState((p) => ({ ...p, sessionCompleted: e.target.value }))} /></label></div></div></> }] },
