...
export default function ThiccFittSection() {
  ...
  const shelves = [
    ...
    { id: 'F', columns: 2, panels: [{ id: 'weekly-battle-tally', token: 'medium', content: <><h2>WEEKLY BATTLE TALLY</h2><div className="tf30-weekly-list">{weeklyBattleRows.map((row) => <div key={row.day} className="tf30-week-row"><strong>{row.day}</strong><span>{row.trained ? 'TRAINED' : 'REST'}</span><span>GYM {row.gymMinutes} MIN</span><span>CARDIO {row.cardioMinutes} MIN</span><span>TOTAL {row.totalMinutes} MIN</span></div>)}</div><div className="tf30-weekly-summary"><span>DAYS TRAINED: {weeklyDaysTrained}</span><span>TOTAL HOURS TRAINED: {weeklyTotalHours}</span></div></> }, { id: 'caffeine-tally', token: 'medium', content: <><h2>UPUP JUICE TRACKER</h2><div className="tf30-upup-rack">{WEEK_DAYS.map((day) => <label key={day} className="tf30-upup-vial"><span>{day}</span><input value={state.weeklyTrackers.byDay[day].caffeineMg} onChange={(e) => updateWeeklyDay(day, 'caffeineMg', e.target.value)} /><small>MG</small></label>)}</div><div className="tf30-weekly-summary tf30-upup-summary"><span>WEEKLY MG TOTAL: {caffeineTotal}</span><span>DAILY AVERAGE MG: {caffeineAvg}</span></div></> }] },
