...
export default function ThiccFittSection() {
  ...
  const tiles = [
    ...
    { id: 'iron-ledger', title: 'IRON.LEDGER', alwaysOpen: true, summary: firstExercise ? `${firstExercise.exercise}\n${firstExercise.sets || '—'} SETS · ${firstExercise.reps || '—'} REPS · ${firstExercise.weight || '—'}` : 'OPEN TO LOG TODAY’S WORKOUT', content: <>{panelById['iron-ledger']}<section className="tf30-vault-inline">{panelById['da-vault']}</section></> },
