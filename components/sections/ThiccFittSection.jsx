  ...
  return (
    <section className="tf-page">
      ...
      <main className="tf-grid">
        ...
        <section className="tf-panel tf-exercises"><h2>EXERCISE LOG</h2>{exerciseRows.slice(0,5).map((row, i) => <div key={i} className="tf-row"><input placeholder="EXERCISE" value={row.exercise} onChange={(e)=>updateExercise(i,'exercise',e.target.value)} /><input placeholder="WEIGHT" value={row.weight} onChange={(e)=>updateExercise(i,'weight',e.target.value)} /><input placeholder="SETS" value={row.sets} onChange={(e)=>updateExercise(i,'sets',e.target.value)} /><input placeholder="REPS" value={row.reps} onChange={(e)=>updateExercise(i,'reps',e.target.value)} /></div>)}<button type="button" onClick={addExercise}>+ ADD EXERCISE</button></section>
