...
export default function ThiccFittSection() {
  ...
  return <section className="tf-page"><div className="tf-overlay" />
    ...
    <main className="tf-grid">
      <section className="tf-panel tf-exercises">
        <h2>EXERCISE LOG</h2>
        {state.exerciseRows.map((row, i) => <div key={i} className="tf-row"><input placeholder="EXERCISE" value={row.exercise} onChange={(e) => updateExercise(i, 'exercise', e.target.value)} /><input placeholder="WEIGHT" value={row.weight} onChange={(e) => updateExercise(i, 'weight', e.target.value)} /><input placeholder="REPS" value={row.reps} onChange={(e) => updateExercise(i, 'reps', e.target.value)} /><input placeholder="SETS" value={row.sets} onChange={(e) => updateExercise(i, 'sets', e.target.value)} /><select value={row.failure} onChange={(e) => updateExercise(i, 'failure', e.target.value)}><option value="">FAILURE Y/N</option><option>Y</option><option>N</option></select></div>)}
        <button className="tf-add-row" type="button" onClick={addExerciseRow}>+ ADD EXERCISE RPM</button>
      </section>

      <section className="tf-panel tf-cardio-mid">
        <h2>CARDIO</h2>
        <div className="tf-row"><select value={state.form.cardioType} onChange={(e) => updateField('cardioType', e.target.value)}><option value="">TYPE</option>{opts.roidCardioType.map((o) => <option key={o}>{o}</option>)}</select><select value={state.form.cardioDuration} onChange={(e) => updateField('cardioDuration', e.target.value)}><option value="">DURATION</option>{opts.roidCardioDuration.map((o) => <option key={o}>{o}</option>)}</select><select value={state.form.cardioIntensity} onChange={(e) => updateField('cardioIntensity', e.target.value)}><option value="">INTENSITY</option>{opts.roidIntensity.map((o) => <option key={o}>{o}</option>)}</select></div>
        <div className="tf-row"><input placeholder="CARDIO LOCATION" value={state.form.cardioLocation} onChange={(e) => updateField('cardioLocation', e.target.value)} /><textarea value={state.form.cardioNotes} onChange={(e) => updateField('cardioNotes', e.target.value)} placeholder="NOTES" /></div>
      </section>

      <section className="tf-panel tf-notes">
        <h2>SO HOW YOU DOIN 🫪⁉️</h2>
        <div className="tf-notes-stack"><select value={state.form.notesPrompt} onChange={(e) => updateField('notesPrompt', e.target.value)}>{opts.approvedPrompts.map((o) => <option key={o}>{o}</option>)}</select><textarea value={state.form.notesText} onChange={(e) => updateField('notesText', e.target.value)} placeholder="WRITE YOUR THOUGHTS HERE..." /></div>
      </section>

      <section className="tf-panel tf-media-main">
        <h2>MEDIA</h2>
        <div className="tf-media-grid"><label className="tf-upload">UPLOAD<input type="file" accept="image/*" onChange={(e) => setMediaPreview([...(mediaPreview.slice(1)), ...(e.target.files?.[0] ? [e.target.files[0]] : [])].slice(0, 3))} /></label>{[0, 1].map((slot) => <label key={slot} className="tf-upload">{previewUrls[slot] ? <img src={previewUrls[slot]} alt="Preview" /> : 'MEDIA'}<input type="file" accept="image/*" onChange={(e) => { if (!e.target.files?.[0]) return; const next = [...mediaPreview]; next[slot + 1] = e.target.files[0]; setMediaPreview(next.slice(0, 3)); }} /></label>)}</div>
      </section>

      <section className="tf-panel tf-left-stack">
        <section className="tf-panel tf-stats"><h2>STATS</h2><div className="tf-stats-scroll"><div className="tf-stats-grid">{statFields.map(([k, l]) => <label key={k} className="tf-labeled"><span>{l}</span><input value={state.form[k]} onChange={(e) => updateField(k, e.target.value)} /></label>)}</div><textarea value={state.form.bodyNotes} onChange={(e) => updateField('bodyNotes', e.target.value)} placeholder="NOTES" /></div></section>

        <section className="tf-panel tf-vault"><h2>THE VAULT</h2><div className="tf-vault-scroll">{state.vaultRows.map((row, i) => <div key={i} className="tf-vault-stack"><label className="tf-labeled"><span>COMPOUND</span><select value={row.compound} onChange={(e) => updateVault(i, 'compound', e.target.value)}><option value=""></option>{opts.roidCompound.map((o) => <option key={o}>{o}</option>)}</select></label><label className="tf-labeled"><span>ESTER</span><select value={row.ester} onChange={(e) => updateVault(i, 'ester', e.target.value)}><option value=""></option>{opts.roidEster.map((o) => <option key={o}>{o}</option>)}</select></label><label className="tf-labeled"><span>AMOUNT</span><select value={row.amount} onChange={(e) => updateVault(i, 'amount', e.target.value)}><option value=""></option>{opts.roidAmount.map((o) => <option key={o}>{o}</option>)}</select></label><label className="tf-labeled tf-inline-fill"><span>SHOT __ OF __</span><input value={row.shotOf} onChange={(e) => updateVault(i, 'shotOf', e.target.value)} /></label><label className="tf-labeled"><span>SENSITIVITY</span><select value={row.sensitivity} onChange={(e) => updateVault(i, 'sensitivity', e.target.value)}><option value=""></option>{opts.roidSensitivity.map((o) => <option key={o}>{o}</option>)}</select></label><label className="tf-labeled tf-inline-fill"><span>CYCLE WEEK __ OF __</span><input value={row.weekOf} onChange={(e) => updateVault(i, 'weekOf', e.target.value)} /></label></div>)}<div className="tf-pump-wrap"><button type="button" className="tf-pump-trigger" onClick={sendToAssurer} onDoubleClick={resetSends} disabled={!canSend} aria-label="Pump It send trigger"><img src="/backgrounds/THICC-FITT/pump-it.PNG" alt="PUMP-IT GLYPH" /></button></div></div></section>

        <section className="tf-panel tf-mantra"><h2>THICC.FITT QUOTE</h2><div className="tf-quote-frame"><select value={state.form.activeQuote} onChange={(e) => updateField('activeQuote', e.target.value)}>{optionRegistry.thiccFitt.quoteOfDay.map((o) => <option key={o}>{o}</option>)}</select><p>{state.form.activeQuote}</p></div></section>
      </section>
    </main>
  </section>;
}
