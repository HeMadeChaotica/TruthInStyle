...
export default function RememberMeSection() {
  ...
  const monthGrid = useMemo(() => {
    ...
    const cells=[]; for (let i=first-1;i>=0;i--) cells.push({day:dip-i,inMonth:false}); for (let d=1; d<=dim; d++) cells.push({day:d,inMonth:true}); while (cells.length%7!==0) cells.push({day:cells.length,inMonth:false}); return cells;
  }, [viewDate]);
