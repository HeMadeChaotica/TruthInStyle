...
const buildMonthGrid = (v) => {
  const y = v.getFullYear(); const m = v.getMonth(); const f = new Date(y, m, 1); const w = f.getDay(); const dim = new Date(y, m + 1, 0).getDate(); const dip = new Date(y, m, 0).getDate(); const cells = [];
  for (let i = w - 1; i >= 0; i--) cells.push({ day: dip - i, inMonth: false, dateKey: toDateKey(y, m - 1, dip - i) });
  for (let d = 1; d <= dim; d++) cells.push({ day: d, inMonth: true, dateKey: toDateKey(y, m, d) });
  for (let t = 1; cells.length % 7 !== 0; t++) cells.push({ day: t, inMonth: false, dateKey: toDateKey(y, m + 1, t) });
  return cells;
};
...
export default function RememberMeSection() {
  ...
  const monthVisual = MONTH_VISUAL_CONFIG[monthIndex]; const monthGrid = useMemo(() => buildMonthGrid(viewDate), [viewDate]); const selectedDateKey = toDateKey(year, monthIndex, selectedDay);
  ...
  const saveMoment = () => { const draft = { ...momentDraft }; if (activeMomentId) { setMomentsByDate((c) => ({ ...c, [selectedDateKey]: (c[selectedDateKey] ?? []).map((m) => m.id === activeMomentId ? { ...m, ...draft } : m) })); } else { const newMoment = { id: crypto.randomUUID(), ...draft, stamped: false }; setMomentsByDate((c) => ({ ...c, [selectedDateKey]: [...(c[selectedDateKey] ?? []), newMoment] })); setActiveMomentId(newMoment.id); } closePostcard(); resetMomentDraft(); };
