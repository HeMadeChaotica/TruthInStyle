...
export default function DaEaterSection() {
  ...
  const shelves = [
    ...
    { id: 'B', columns: 1, panels: [{ id: 'meals', token: 'medium', className: 'da-eater-panel', content: <><h2>MEAL LOG</h2><div className="da-eater-meal-log-layout"><div className="da-eater-meal-input-stack"><select value={mealForm.type} onChange={(e)=>setMealForm({...mealForm,type:e.target.value})}>{MEAL_TYPES.map((x)=><option key={x}>{x}</option>)}</select><input placeholder="MEAL NAME" value={mealForm.name} onChange={(e)=>setMealForm({...mealForm,name:e.target.value})}/><input type="time" value={mealForm.time} onChange={(e)=>setMealForm({...mealForm,time:e.target.value})}/>{mealForm.type === 'WATER' ? <input placeholder="WATER AMOUNT (OZ)" value={mealForm.waterOz || ''} onChange={(e)=>setMealForm({...mealForm,waterOz:e.target.value})}/> : <><input placeholder="PROTEIN" value={mealForm.protein} onChange={(e)=>setMealForm({...mealForm,protein:e.target.value})}/><input placeholder="CARBS" value={mealForm.carbs} onChange={(e)=>setMealForm({...mealForm,carbs:e.target.value})}/><input placeholder="FATS" value={mealForm.fats} onChange={(e)=>setMealForm({...mealForm,fats:e.target.value})}/><input placeholder="CALORIES" value={mealForm.calories} onChange={(e)=>setMealForm({...mealForm,calories:e.target.value})}/></>}<button onClick={saveMeal}>SAVE MEAL</button></div><div className="da-eater-meal-saved-list"><h3>SAVED MEALS</h3>{(day.meals||[]).length===0 ? <p className="da-eater-empty">NO SAVED MEALS YET.</p> : (day.meals||[]).map((m)=><div key={m.id} className="da-eater-meal-card"><div className="da-eater-meal-card-main"><strong>{m.type} • {m.name || 'UNTITLED'}</strong><span>{m.time || 'TIME NOT SET'}</span><div className="da-eater-meal-macros">{m.type === 'WATER' ? <span>WATER {m.waterOz || 0}OZ</span> : <><span>P {m.protein || 0}</span><span>C {m.carbs || 0}</span><span>F {m.fats || 0}</span><span>KCAL {m.calories || 0}</span></>}</div></div><div className="da-eater-meal-controls"><button onClick={()=>setMealForm(m)}>EDIT</button><button onClick={()=>setDay(deleteMealEntry(date,m.id))}>DELETE</button></div></div>)}</div></div></> }] },
