...
export default function ItsGettingThiccSection() {
  ...
  const vaultCompoundOptions = useClockItOptions(CLOCK_IT_KEYS.itsVaultCompound);
  const vaultEsterOptions = useClockItOptions(CLOCK_IT_KEYS.itsVaultEster);
  const vaultAmountCcOptions = useClockItOptions(CLOCK_IT_KEYS.itsVaultAmountCc);
  const vaultAmountMgOptions = useClockItOptions(CLOCK_IT_KEYS.itsVaultAmountMg);
  const vaultSensitivityOptions = useClockItOptions(CLOCK_IT_KEYS.itsVaultSensitivity);
  ...
  const infoShelves = [
    ...
    { id: 'D', columns: 4, panels: [
      ...
      { id: 'vault', token: 'standard', content: <><h3>VAULT</h3><label>COMPOUND<select value={infoClient.juice_substance || ''} onChange={(e) => update('juice_substance', e.target.value)}><option value="">SELECT</option>{vaultCompoundOptions.map((option) => <option key={option}>{option}</option>)}</select></label><label>ESTER / FORM<select value={infoClient.juice_ester || ''} onChange={(e) => update('juice_ester', e.target.value)}><option value="">SELECT</option>{vaultEsterOptions.map((option) => <option key={option}>{option}</option>)}</select></label><label>AMOUNT<select value={infoClient.juice_amount || ''} onChange={(e) => update('juice_amount', e.target.value)}><option value="">SELECT</option><optgroup label="CC">{vaultAmountCcOptions.map((option) => <option key={option}>{option}</option>)}</optgroup><optgroup label="MG">{vaultAmountMgOptions.map((option) => <option key={option}>{option}</option>)}</optgroup></select></label><label>SHOT __ OF __<input value={infoClient.juice_shot || ''} onChange={(e) => update('juice_shot', e.target.value)} /></label><label>SENSITIVITY / SIDE EFFECTS<select value={infoClient.juice_sensitivity || ''} onChange={(e) => update('juice_sensitivity', e.target.value)}><option value="">SELECT</option>{vaultSensitivityOptions.map((option) => <option key={option}>{option}</option>)}</select></label><label>CYCLE WEEK __ OF __<input value={infoClient.juice_cycle || ''} onChange={(e) => update('juice_cycle', e.target.value)} /></label><label>LOCATION<input value={infoClient.juice_location || ''} onChange={(e) => update('juice_location', e.target.value)} /></label><label>NOTES<textarea value={infoClient.juice_notes || ''} onChange={(e) => update('juice_notes', e.target.value)} /></label></> },
  ...
  const entryColor = (entry = {}) => {
    try {
      ...
      return resolveClientColor(safeKey).value;
    } catch (error) {
