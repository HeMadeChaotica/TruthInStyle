...
export default function ItsGettingThiccSection() {
  ...
  const loadCalendar = async () => {
    try {
      ...
      setCalendar(safeRows);
      return safeRows;
    } catch (error) {
      console.error('[THICC.TIME LOAD]', error);
      setTimeError(error?.message || 'THICC.TIME LOAD FAILED');
      setCalendar([]);
      return [];
    }
  };
