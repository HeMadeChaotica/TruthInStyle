...
export default function ItsGettingThiccSection() {
  ...
  return <section className="igt-page"><div className="igt-shell">
    ...
    <main className="igt-grid">
      ...
      <section className="igt-panel hz"><h3>TRAINING / REST CALENDAR</h3><div className='igt-hz-days'>{days.map((d,i)=><div className='day' key={d}><span>{d}</span><select value={active.trainingRest[i]} onChange={(e)=>updateArray('trainingRest',i,e.target.value)}><option>TRAINING</option><option>REST</option></select></div>)}</div></section>
      <section className="igt-panel hz"><h3>CURRENT EXERCISE PROGRAM SPLIT</h3><div className='igt-hz-days'>{days.map((d,i)=><div className='day' key={d}><span>{d}</span><select value={active.programSplit[i]} onChange={(e)=>updateArray('programSplit',i,e.target.value)}><option>FULLBODY</option><option>UPPER</option><option>LOWER</option></select></div>)}</div></section>
