...
export default function ItsGettingThiccSection() {
  ...
  return <section className="igt-page"><div className="igt-shell">
    ...
    <main className="igt-grid">
      ...
      <div className="row"><select value={active.sex} onChange={(e)=>update('sex',e.target.value)}><option value="">SEX</option>{optionRegistry.itsGettingThicc.sex.map((o)=><option key={o}>{o}</option>)}</select>
      <select value={active.orientation} onChange={(e)=>update('orientation',e.target.value)}><option value="">SEXUAL ORIENTATION</option>{optionRegistry.itsGettingThicc.sexualOrientation.map((o)=><option key={o}>{o}</option>)}</select></div>
      ...
      <section className="igt-panel compact"><h3>BODY / GOAL</h3>{['currentWeight','goalWeight','currentBmi','goalBmi'].map((k)=><input key={k} placeholder={k.replace(/([A-Z])/g, ' $1').toUpperCase().trim()} value={active[k]} onChange={(e)=>update(k,e.target.value)} />)}</section>
      <section className="igt-panel compact"><h3>AT A GLANCE</h3><div className="glance">{glancePhoto && <img src={glancePhoto} alt="glance"/>}<p>HEIGHT: {active.heightFt}'{active.heightIn}" {active.heightCm && `(${active.heightCm}CM)`}</p><p>CURRENT WEIGHT: {active.currentWeight}</p><p>GOAL WEIGHT: {active.goalWeight}</p><p>CURRENT BMI: {active.currentBmi}</p><p>GOAL BMI: {active.goalBmi}</p></div></section>
      <section className="igt-panel wide pin-scroll"><h3>FOOD: THE GOOD, THE BAD & THE “I DESERVE THIS”</h3>{[1,2,3,4,5].map((n)=><div key={n} className="igt-question"><label>{['WHAT DO YOU STRUGGLE THE HARDEST TO STAY AWAY FROM?','WHEN DO YOU USUALLY REACH FOR IT? (TIME, MOOD, SITUATION)','GROWING UP, WERE YOU FORCED TO FINISH YOUR PLATE, AND HOW HAS THAT AFFECTED YOUR EATING HABITS IN ADULTHOOD?','WERE THERE FOOD RULES, PUNISHMENTS, PRESSURE, OR EMOTIONALLY CHARGED EXPERIENCES AROUND EATING GROWING UP THAT STILL AFFECT HOW YOU EAT NOW?','HOW DO YOUR EMOTIONS AND FOOD INTERACT TODAY?'][n-1]}</label><input value={active[`food${n}`]} onChange={(e)=>update(`food${n}`,e.target.value)} /></div>)}</section>
      <section className="igt-panel wide pin-scroll"><h3>MOVEMENT: THE MEASURE AND THE PRESSURES</h3>{[1,2,3,4].map((n)=><input key={n} value={active[`move${n}`]} onChange={(e)=>update(`move${n}`,e.target.value)} placeholder={['WALK ME THROUGH A TYPICAL DAY OF EATING.','WALK ME THROUGH A NORMAL DAY IN YOUR BODY.','HOW MUCH OF YOUR DAY IS SITTING VS ACTUALLY MOVING?','WHEN DO YOU FEEL MOST PHYSICALLY ALIVE?'][n-1]} />)}<select value={active.activity} onChange={(e)=>update('activity',e.target.value)}><option value=''>ACTIVITY</option>{optionRegistry.itsGettingThicc.activityLevel.map(o=><option key={o}>{o}</option>)}</select></section>
      <section className="igt-panel wide pin-scroll"><h3>MEDICAL ADVISORY</h3>{[['medEmergency','EMERGENCY CONTACT NAME / NUMBER'],['medInjuries','PAST / CURRENT INJURIES'],['medSurgeries','PAST / UPCOMING SURGERIES'],['medAllergies','RECURRING SEASONAL ISSUES / ALLERGIES'],['medMeds','MEDICATIONS THAT COULD CAUSE COMPLICATIONS'],['medLimits','PHYSICAL LIMITATIONS'],['medPain','MOVEMENTS THAT CAUSE PAIN'],['medHardNo',"HARD NO'S"],['medFears','TRAINING FEARS']].map(([k,l])=><input key={k} placeholder={l} value={active[k]} onChange={(e)=>update(k,e.target.value)} />)}<select value={active.medFlex || ''} onChange={(e)=>update('medFlex',e.target.value)}><option value=''>FLEXIBILITY LEVEL</option>{optionRegistry.itsGettingThicc.flexibilityLevel.map((o)=><option key={o}>{o}</option>)}</select>{}</section>
      ...
      <section className="igt-panel compact-small"><h3>SEASONS PER WEEK</h3><input type="number" value={active.seasonsPerWeek} onChange={(e)=>update('seasonsPerWeek',e.target.value)} /></section>
      <section className="igt-panel"><h3>REFERRAL TRACKER</h3><select value={active.referralStatus || ''} onChange={(e)=>update('referralStatus',e.target.value)}><option value=''>REFERRAL STATUS</option>{optionRegistry.itsGettingThicc.referralStatus.map((o)=><option key={o}>{o}</option>)}</select>{active.referrals.map((r,i)=><div className="row" key={i}><input placeholder='NAME' value={r.name} onChange={(e)=>update('referrals',active.referrals.map((x,idx)=>idx===i?{...x,name:e.target.value}:x))}/><input type='date' value={r.date} onChange={(e)=>update('referrals',active.referrals.map((x,idx)=>idx===i?{...x,date:e.target.value}:x))}/><input placeholder='BONUS WEEK' value={r.bonusWeek} onChange={(e)=>update('referrals',active.referrals.map((x,idx)=>idx===i?{...x,bonusWeek:e.target.value}:x))}/></div>)}<button onClick={()=>update('referrals',[...active.referrals,{name:'',date:'',bonusWeek:''}])}>ADD REFERRAL</button></section>
      <section className="igt-panel hz"><h3>TRAINING / REST CALENDAR</h3><div className='igt-hz-days'>{days.map((d,i)=><div className='day' key={d}><span>{d}</span><select value={active.trainingRest[i]} onChange={(e)=>updateArray('trainingRest',i,e.target.value)}>{optionRegistry.itsGettingThicc.trainingRest.map((o)=><option key={o}>{o}</option>)}</select></div>)}</div></section>
      <section className="igt-panel hz"><h3>CURRENT EXERCISE PROGRAM SPLIT</h3><div className='igt-hz-days'>{days.map((d,i)=><div className='day' key={d}><span>{d}</span><select value={active.programSplit[i]} onChange={(e)=>updateArray('programSplit',i,e.target.value)}>{optionRegistry.itsGettingThicc.programSplit.map((o)=><option key={o}>{o}</option>)}</select></div>)}</div></section>
      <section className="igt-panel"><h3>UPCOMING TRAINING FOCUS EVENTS</h3>{active.upcomingEvents.map((ev,i)=><div className='row' key={i}><select value={ev.type} onChange={(e)=>update('upcomingEvents',active.upcomingEvents.map((x,idx)=>idx===i?{...x,type:e.target.value}:x))}>{optionRegistry.itsGettingThicc.upcomingTrainingEventType.map((o)=><option key={o}>{o}</option>)}</select>{ev.type==='CUSTOM'&&<input value={ev.custom} onChange={(e)=>update('upcomingEvents',active.upcomingEvents.map((x,idx)=>idx===i?{...x,custom:e.target.value}:x))} placeholder='CUSTOM EVENT'/>}<input type='date' value={ev.date} onChange={(e)=>update('upcomingEvents',active.upcomingEvents.map((x,idx)=>idx===i?{...x,date:e.target.value}:x))}/></div>)}</section>
      <section className="igt-panel"><h3>PAYMENT</h3><select value={active.paymentSchedule} onChange={(e)=>update('paymentSchedule',e.target.value)}><option value=''>SCHEDULE</option>{optionRegistry.itsGettingThicc.paymentSchedule.map((o)=><option key={o}>{o}</option>)}</select><input type='date' value={active.paymentDate} onChange={(e)=>update('paymentDate',e.target.value)} /><input placeholder='NOTES' value={active.paymentNotes} onChange={(e)=>update('paymentNotes',e.target.value)} /></section>
      ...
      <section className="igt-panel full"><div className='row'><h3>CELEBRATION MOMENTS</h3><button className='delete-inline' onClick={()=>{ if(!confirm('DELETE CURRENT CLIENT FILE?')) return; const next=clients.filter(c=>c.id!==active.id); persist(next,'delete-client'); setActiveId(next[0]?.id||''); }}>DELETE</button></div><div className='cele'>{active.celebration.map((img,i)=><label key={i} className='slot'>{img?<img src={img} alt=''/>:`${i+1}`}<input type='file' accept='image/*' onChange={onUpload(String(i))}/></label>)}</div></section>
    </main>
  </div></section>;
}
