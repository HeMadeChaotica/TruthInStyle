...
export default function ItsGettingThiccSection() {
  ...
  const normalizeSchedulePayload = ({ currentEntryForm, currentActive, currentScheduleType }) => {
    ...
    const dayIndex = new Date(safeDate).getDay();
    const safeDay = Number.isInteger(dayIndex) && dayIndex >= 0 && dayIndex < days.length ? days[dayIndex] : days[now.getDay()];
    const baseLabel = currentActive?.programSplit?.[dayIndex];
