...
create table if not exists public.thicc_client_profiles (
  client_id uuid primary key references public.thicc_clients(id) on delete cascade,
...
create table if not exists public.thicc_client_program_splits (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,version_label text,is_active boolean default true,split_days_json jsonb default '[]'::jsonb,training_rest_days_json jsonb default '[]'::jsonb,seasons_per_week integer,start_date date,end_date date,notes text,created_at timestamptz default now());
create table if not exists public.thicc_client_schedule_entries (id uuid primary key default gen_random_uuid(),client_id uuid references public.thicc_clients(id) on delete set null,entry_type text not null,entry_date date not null,start_time time,end_time time,workout_label text,source_split_day text,location text,notes text,color_option_key text,created_at timestamptz default now(),updated_at timestamptz default now());
...
create table if not exists public.thicc_client_form_assignments (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,form_id uuid not null references public.thicc_client_forms(id) on delete cascade,status text not null default 'assigned',assigned_at timestamptz default now(),completed_at timestamptz,response_json jsonb default '{}'::jsonb,notes text);
create table if not exists public.thicc_client_referrals (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,referral_name text not null,referral_date date,status text,notes text);
create table if not exists public.thicc_client_focus_events (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,event_type text,event_label text,event_date date,notes text,active boolean default true);
create table if not exists public.thicc_client_payments (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,payment_schedule text,payment_due_date date,amount numeric,status text,notes text,created_at timestamptz default now());
create table if not exists public.thicc_client_celebrations (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,title text not null,description text,media_id text,sort_order integer default 0,created_at timestamptz default now());
create table if not exists public.thicc_client_checkins (id uuid primary key default gen_random_uuid(),client_id uuid not null references public.thicc_clients(id) on delete cascade,meals_per_week integer,myfitfoods_verified boolean default false,protein_target numeric,carb_target numeric,fat_target numeric,water_target numeric,calorie_target numeric,notes text,created_at timestamptz default now());
